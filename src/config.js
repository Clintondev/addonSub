...
module.exports = Object.freeze({
  ...
  contextualTranslatorMaxCues: number("CONTEXTUAL_TRANSLATOR_MAX_CUES", 24, { min: 4, max: 80 }),
  contextualTranslatorContextCues: number("CONTEXTUAL_TRANSLATOR_CONTEXT_CUES", 12, { min: 2, max: 40 }),
  ...
  internalHttpTimeoutMs: number("INTERNAL_HTTP_TIMEOUT_SECONDS", 30, { min: 2, max: 300 }) * 1000,
  transcriptionTimeoutMs: number("TRANSCRIPTION_TIMEOUT_MINUTES", 120, { min: 5, max: 720 }) * 60 * 1000,
  mediaProcessTimeoutMs: number("MEDIA_PROCESS_TIMEOUT_MINUTES", 180, { min: 5, max: 720 }) * 60 * 1000,
  storageReservationLeaseMs: number("STORAGE_RESERVATION_LEASE_SECONDS", 300, { min: 30, max: 3600 }) * 1000,
